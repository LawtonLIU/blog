'''大概步骤 不同oauth商可能有不同要求
用户授权登录
oauth返回code
网站需携带code向oauth请求令牌
oauth返回access_token
网站携带access_token可以向oauth请求用户信息
'''
import json
import os
import urllib.parse
...
class WBOauthManager(BaseOauthManager):
    ...
    def get_access_token_by_code(self,code):
        ...
        if 'access_token' in obj:
            self.access_token=str(obj['uid'])
            return self.get_oauth_userinfo()
        else:
            raise OAuthAccessTokenException
    ...
    def get_oauth_userinfo(self):
        ...
        try:
            ...
            user.nikcname=datas['screen_name']
...
class ProxyManagerMixin:
    def __init__(self,*args,**kwargs):
        if os.environ.get('HTTP_PROXY'): # 获取环境变量
            self.proxies={
...
class GitHubOauthManager(ProxyManagerMixin,BaseOauthManager):
    ...
    def get_authorization_url(self,next_url='/'):
        params={
            'client_id':self.client_id,
            'response_type':'code',
            'redirect_url':f'{self.callback_url}&next_url={next_url}',
            'scope':'user'
        }
        url=self.AUTH_URL+'?'+urllib.parse.urlencode(params)
        return url
...
def get_oauth_app():
    ...
    if not configs:
        return []
    configtypes=[x.type for x in configs] # 类别列表
    applications=BaseOauthManager.__subclasses__() # 子类
    apps=[x() for x in applications if x().ICON_NAME.lower() in configtypes] # 返回在configtypes中的apps
    return apps
    
def get_manager_by_type(type):
    applications=get_oauth_app() # 获取apps列表
    if applications:
        finds=list(
            filter(lambda x:x.ICON_NAME.lower() == type.lower(),applications)) # 看看type是否存在
        if finds:
            return finds[0]
    return None
