...
def authorize(request):
    ...
    try:
        ...
        print('OAuthAccessTokenException'+str(e))
        return HttpResponseRedirect('/')        
    ...
    if user:
        ...
        if user.email:
            ...
            user.save()
            url=reverse('oauth:require_email',kwargs={'authorid':user.id})
            return HttpResponseRedirect(url)
    else:
        return HttpResponseRedirect(nexturl)
    
def emailconfim(request,id,sign):
    if not sign:
    ...
    with transaction.atomic():
        if oauthuser.author:
            ...
            if result[1]:
                ...
                author.save()
        oauthuser.save()
    oauth_user_login_signal.send(sender=emailconfim.__class__,id=oauthuser.id)
    login(request,author)
...
class RequireEmailView(FormView):
    ...
    template_name='oauth/required_email.html'
